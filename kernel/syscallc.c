...
{
	//return p_proc_current->task.pid;	//deleted by mingxuan 2019-1-16
	return proc->task.pid;	//modified by mingxuan 2019-1-16
}
...
{	
	...
	for( AddrLin=vir_addr; AddrLin<vir_addr+size ; AddrLin += num_4B )//一个字节一个字节处理
	{
		lin_mapping_phy(	AddrLin,//线性地址					//add by visual 2016.5.9
							...
							//p_proc_current->task.pid,//进程pid	//deleted by mingxuan 2019-1-16
							proc->task.pid,	//进程pid	//modified by mingxuan 2019-1-16
...
{//线性地址可以不释放，但是页表映射关系必须清除！
	...
	//phy_addr = get_page_phy_addr(p_proc_current->task.pid,(int)AddrLin);//获取物理页的物理地址 //deleted by mingxuan 2019-1-16
	phy_addr = get_page_phy_addr(proc->task.pid,(int)AddrLin);//获取物理页的物理地址 //modified by mingxuan 2019-1-16
