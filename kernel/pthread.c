...
PUBLIC int sys_pthread(void *entry)
{
	...
	//acquire(&pthreadlock); //added by mingxuan 2019-3-14
	/*****************申请空白PCB表**********************/
	p_child = alloc_PCB();
	if( 0==p_child )
	{
	...
	{	
		PROCESS *p_parent;
		//if( p_proc_current->task.info.type == TYPE_THREAD )	//deleted by mingxuan 2019-1-16
		if( proc->task.info.type == TYPE_THREAD )	//modified by mingxuan 2019-1-16
		{//线程
			//p_parent = &(proc_table[p_proc_current->task.info.ppid]);//父进程 //deleted by mingxuan 2019-1-16
			p_parent = &(proc_table[proc->task.info.ppid]);//父进程 //modified by mingxuan 2019-1-16
		}
		else
		{//进程
			//p_parent = p_proc_current;//父进程就是父线程
			p_parent = proc;//父进程就是父线程
		}
		/************复制父进程的PCB部分内容（保留了自己的标识信息,但cr3使用的是父进程的）**************/
		pthread_pcb_cpy(p_child,p_parent);
		

		/************在父进程的栈中切割分配子线程的栈（从进程栈的低地址分配8M,注意方向）**********************/
	...
	//release(&pthreadlock); //added by mingxuan 2019-3-14
...
PRIVATE int pthread_update_info(PROCESS* p_child,PROCESS *p_parent)
{
	...
	//if( p_parent!=p_proc_current ) //deleted by mingxuan 2019-1-16
	if( p_parent!=proc ) //modified by mingxuan 2019-1-16
	...
	//p_proc_current->task.info.child_t_num += 1;	//子线程数量 //deleted by mingxuan 2019-1-16
	proc->task.info.child_t_num += 1;	//子线程数量 //modified by mingxuan 2019-1-16
	//p_proc_current->task.info.child_thread[p_proc_current->task.info.child_t_num-1] = p_child->task.pid;//子线程列表
	proc->task.info.child_thread[proc->task.info.child_t_num-1] = p_child->task.pid;//子线程列表 //modified by mingxuan 2019-1-16		
	...
	p_child->task.info.real_ppid = proc->task.pid;  //亲父进程，创建它的那个线程，注意，这个是创建它的那个线程proc //modified by mingxuan 2019-1-16
