...
//PRIVATE int insert_cpu_ready_proc(u32 pid, u32 cpuid);
...
PUBLIC int sys_fork()
{
	...
	//acquire(&forklock);	//added by mingxuan 2019-3-12
	...
	else
	{
		...
		/**************复制线性内存，包括堆、栈、代码数据等等***********************/
		//fork_mem_cpy(p_proc_current->task.pid,p_child->task.pid);	//deleted by mingxuan 2019-1-16
		fork_mem_cpy(proc->task.pid, p_child->task.pid);	//modified by mingxuan 2019-1-16
		...
		//insert_cpu_ready_proc(p_child->task.pid, cpu->id);	 //added by mingxuan 2019-2-28
		add_ready_queue(&cpu->ready, p_child);	 //modified by mingxuan 2019-4-3
		cpu->nr_ready_proc++; //该cpu当前运行的进程的个数，作为负载均衡的参数

		/************修改子进程的名字***************/		
		strcpy(p_child->task.p_name, "fork");	// 所有的子进程都叫fork
		
		/*************子进程返回值在其eax寄存器***************/             
		p_child->task.regs.eax = 0;		//return child with 0 	//这句话是什么意思？ mingxuan
		...
		*((u32*)(p_reg + EAXREG - P_STACKTOP)) = p_child->task.regs.eax;	//added by xw, 17/12/11
		...
		//disp_color_str(p_proc_current->task.p_name,0x72); //deleted by mingxuan 2019-1-16
		//disp_color_str(proc->task.p_name,0x72); //modified by mingxuan 2019-1-16
		//disp_str("-");
		disp_int(proc->task.pid);	//added by mingxuan 2019-3-1
		disp_str("->");
		disp_int(p_child->task.pid);	//added by mingxuan 2019-3-1
		disp_color_str("]",0x72);
		...
		release(&allocPCBlock);	//为了避免PCB被重复申请	//added by mingxuan 2019-3-14
...
PRIVATE int fork_mem_cpy(u32 ppid,u32 pid)
{
	...
	for(addr_lin = proc->task.memmap.text_lin_base ; addr_lin < proc->task.memmap.text_lin_limit ; addr_lin+=num_4K ) //modified by mingxuan 2019-1-16
	{
	...
	for(addr_lin = proc->task.memmap.data_lin_base ; addr_lin < proc->task.memmap.data_lin_limit ; addr_lin+=num_4K ) //modified by mingxuan 2019-1-16
	{	
	...
	for(addr_lin = proc->task.memmap.vpage_lin_base ; addr_lin < proc->task.memmap.vpage_lin_limit ; addr_lin+=num_4K ) //modified by mingxuan 2019-1-16
	{
		lin_mapping_phy(SharePageBase,0,ppid,PG_P  | PG_USU | PG_RWW,0);//使用前必须清除这个物理页映射
		lin_mapping_phy(SharePageBase,MAX_UNSIGNED_INT,ppid,PG_P  | PG_USU | PG_RWW,PG_P  | PG_USU | PG_RWW);//利用父进程的共享页申请物理页
		memcpy((void*)SharePageBase,(void*)(addr_lin&0xFFFFF000),num_4K);//将数据复制到物理页上,注意这个地方是强制一页一页复制的
	...
	for(addr_lin = proc->task.memmap.heap_lin_base ; addr_lin < proc->task.memmap.heap_lin_limit ; addr_lin+=num_4K ) //modified by mingxuan 2019-1-16
	{
		lin_mapping_phy(SharePageBase,0,ppid,PG_P  | PG_USU | PG_RWW,0);//使用前必须清除这个物理页映射
		lin_mapping_phy(SharePageBase,MAX_UNSIGNED_INT,ppid,PG_P  | PG_USU | PG_RWW,PG_P  | PG_USU | PG_RWW);//利用父进程的共享页申请物理页
		memcpy((void*)SharePageBase,(void*)(addr_lin&0xFFFFF000),num_4K);//将数据复制到物理页上,注意这个地方是强制一页一页复制的
	...
	for(addr_lin = proc->task.memmap.stack_lin_base ; addr_lin > proc->task.memmap.stack_lin_limit ; addr_lin-=num_4K ) //modified by mingxuan 2019-1-16
	{
		lin_mapping_phy(SharePageBase,0,ppid,PG_P  | PG_USU | PG_RWW,0);//使用前必须清除这个物理页映射
		lin_mapping_phy(SharePageBase,MAX_UNSIGNED_INT,ppid,PG_P  | PG_USU | PG_RWW,PG_P  | PG_USU | PG_RWW);//利用父进程的共享页申请物理页
		memcpy((void*)SharePageBase,(void*)(addr_lin&0xFFFFF000),num_4K);//将数据复制到物理页上,注意这个地方是强制一页一页复制的
	...
	for(addr_lin = proc->task.memmap.arg_lin_base ; addr_lin < proc->task.memmap.arg_lin_limit ; addr_lin+=num_4K ) //modified by mingxuan 2019-1-16
	{
		lin_mapping_phy(SharePageBase,0,ppid,PG_P  | PG_USU | PG_RWW,0);//使用前必须清除这个物理页映射
		lin_mapping_phy(SharePageBase,MAX_UNSIGNED_INT,ppid,PG_P  | PG_USU | PG_RWW,PG_P  | PG_USU | PG_RWW);//利用父进程的共享页申请物理页
		memcpy((void*)SharePageBase,(void*)(addr_lin&0xFFFFF000),num_4K);//将数据复制到物理页上,注意这个地方是强制一页一页复制的
...
PRIVATE int fork_pcb_cpy(PROCESS* p_child)
{
	...
	//p_child->task = p_proc_current->task; //deleted by mingxuan 2019-1-16
	p_child->task = proc->task; 			//modified by mingxuan 2019-1-16
	...
	//memcpy(((char*)(p_child + 1) - P_STACKTOP), ((char*)(p_proc_current + 1) - P_STACKTOP), 18 * 4); //deleted by mingxuan 2019-1-16
	memcpy(((char*)(p_child + 1) - P_STACKTOP), ((char*)(proc + 1) - P_STACKTOP), 18 * 4); 			   //modified by mingxuan 2019-1-16
...
PRIVATE int fork_update_info(PROCESS* p_child)
{
	...
	*/
	...
	proc->task.info.child_p_num += 1; //子进程数量
	proc->task.info.child_process[proc->task.info.child_p_num-1] = p_child->task.pid;//子进程列表
	...
	*/
	...
	//modified by mingxuan 2019-1-16
	p_child->task.info.type = proc->task.info.type;	//当前进程属性跟父进程一样
	p_child->task.info.real_ppid = proc->task.pid;  //亲父进程，创建它的那个进程
	p_child->task.info.ppid = proc->task.pid;		//当前父进程
