...
{
	Elf32_Ehdr Echo_Ehdr;	  //ELF header,即ELF头
	Elf32_Phdr Echo_Phdr[10]; //program header table,即程序头表
	Elf32_Shdr Echo_Shdr[10]; //section header table,即段头表
	...
	//acquire(&execlock);	//added by mingxuan 2019-3-7
	...
	/*******************打开文件************************/
	// u32 fd = open(path,"r"); 
	// 目前是直接读取了提前在loader.asm中标记好的init.bin文件所在的内存
	u32 fd = fake_open(path,"r");	//modified by xw, 18/5/30  //目前没有什么用的open
	
	/*************获取elf信息**************/
	//获取该文件的 ELF文件头、program头信息
	...
	/*************根据elf的program复制文件信息**************/
	//根据 ELF文件头、program头的描述申请内存并部署init进程。在部署过程中exec找到未初始化的数据段并进行置0
	...
	proc->task.regs.eip = Echo_Ehdr.e_entry;//进程入口线性地址
	p_reg = (char*)(proc + 1);	//added by xw, 17/12/11
	*((u32*)(p_reg + EIPREG - P_STACKTOP)) = proc->task.regs.eip;	//added by xw, 17/12/11
	
	//栈
	proc->task.regs.esp=(u32)proc->task.memmap.stack_lin_base;		//栈地址最高处
	*((u32*)(p_reg + ESPREG - P_STACKTOP)) = proc->task.regs.esp;	//added by xw, 17/12/11
	
	for( addr_lin=proc->task.memmap.stack_lin_base ; addr_lin > proc->task.memmap.stack_lin_limit ; addr_lin-=num_4K )
	{
		err_temp = lin_mapping_phy(	addr_lin,//线性地址						//add by visual 2016.5.9
									MAX_UNSIGNED_INT,//物理地址						//edit by visual 2016.5.19
									proc->task.pid,//进程pid			//edit by visual 2016.5.19
	...
	/* //deleted by mingxuan 2019-4-10
	disp_str("\n");
	disp_color_str("[",0x72);
	disp_int(cpu->id);	//added by mingxuan 2019-2-27
	disp_str("-");
	...
	*/
	...
	//release(&execlock);	//added by mingxuan 2019-3-7
...
PRIVATE u32 exec_elfcpy(u32 fd,Elf32_Phdr Echo_Phdr,u32 attribute)  // 这部分代码将来要移动到exec.c文件中，包括下面exec()中的一部分
{
	...
	{	
		...
		lin_mapping_phy(lin_addr,MAX_UNSIGNED_INT,proc->task.pid,PG_P  | PG_USU | PG_RWW/*说明*/,attribute);//说明：PDE属性尽量为读写，因为它要映射1024个物理页，可能既有数据，又有代码	//modified by mingxuan 2019-1-16
...
PRIVATE u32 exec_load(u32 fd,const Elf32_Ehdr* Echo_Ehdr,const Elf32_Phdr Echo_Phdr[])
{
	...
	for( ph_num=0; ph_num<Echo_Ehdr->e_phnum ; ph_num++ )
	{
		...
		{//.text
			...
			*/
			//modified by mingxuan 2019-1-16
			proc->task.memmap.text_lin_base = Echo_Phdr[ph_num].p_vaddr;	
			proc->task.memmap.text_lin_limit = Echo_Phdr[ph_num].p_vaddr + Echo_Phdr[ph_num].p_memsz;
		}
		...
		{//.data
			...
			*/
			//modified by mingxuan 2019-1-16
			proc->task.memmap.data_lin_base = Echo_Phdr[ph_num].p_vaddr;
			proc->task.memmap.data_lin_limit = Echo_Phdr[ph_num].p_vaddr + Echo_Phdr[ph_num].p_memsz;
...
PRIVATE int exec_pcb_init(char* path)
{
	...
	*/
	...
	//modified by mingxuan 2019-1-16
	//名称 状态 特权级 寄存器
	strcpy(proc->task.p_name, path);		//名称
	proc->task.stat = READY;  						//状态
	proc->task.ldts[0].attr1 = DA_C | PRIVILEGE_USER << 5;//特权级修改为用户级
	proc->task.ldts[1].attr1 = DA_DRW | PRIVILEGE_USER << 5;//特权级修改为用户级
	proc->task.regs.cs	= ((8 * 0) & SA_RPL_MASK & SA_TI_MASK)| SA_TIL | RPL_USER;
	proc->task.regs.ds	= ((8 * 1) & SA_RPL_MASK & SA_TI_MASK)| SA_TIL | RPL_USER;
	proc->task.regs.es	= ((8 * 1) & SA_RPL_MASK & SA_TI_MASK)| SA_TIL | RPL_USER;
	//proc->task.regs.fs	= ((8 * 1) & SA_RPL_MASK & SA_TI_MASK)| SA_TIL | RPL_USER;	//deleted by mingxuan 2019-2-27
	proc->task.regs.fs	= (SELECTOR_KERNEL_FS & SA_RPL_MASK)| RPL_USER; //modified by mingxuan 2019-1-11
																			//为了[fs:0]表示当前cpu，[fs:4]表示当前进程	
	proc->task.regs.ss	= ((8 * 1) & SA_RPL_MASK & SA_TI_MASK)| SA_TIL | RPL_USER;
	proc->task.regs.gs	= (SELECTOR_KERNEL_GS & SA_RPL_MASK)| RPL_USER;
	proc->task.regs.eflags = 0x202; // IF=1,bit2 永远是1 

	/***************copy registers data****************************/
	//copy registers data to the bottom of the new kernel stack
	//added by xw, 17/12/11
	//p_regs = (char*)(p_proc_current + 1);	//deleted by mingxuan 2019-1-16
	p_regs = (char*)(proc + 1); //modified by mingxuan 2019-1-16
	p_regs -= P_STACKTOP;
	//memcpy(p_regs, (char*)p_proc_current, 18 * 4); //deleted by mingxuan 2019-1-16
	memcpy(p_regs, (char*)proc, 18 * 4); //modified by mingxuan 2019-1-16
	...
	*/
	...
	//modified by mingxuan 2019-1-16
	//进程表线性地址布局部分，text、data已经在前面初始化了
	proc->task.memmap.vpage_lin_base = VpageLinBase;					//保留内存基址
	proc->task.memmap.vpage_lin_limit = VpageLinBase;					//保留内存界限
	proc->task.memmap.heap_lin_base = HeapLinBase;						//堆基址
	proc->task.memmap.heap_lin_limit = HeapLinBase;						//堆界限	
	proc->task.memmap.stack_child_limit = StackLinLimitMAX;		//add by visual 2016.5.27
	proc->task.memmap.stack_lin_base = StackLinBase;						//栈基址
	proc->task.memmap.stack_lin_limit = StackLinBase - 0x4000;			//栈界限（使用时注意栈的生长方向）
	proc->task.memmap.arg_lin_base = ArgLinBase;						//参数内存基址
	proc->task.memmap.arg_lin_limit = ArgLinBase;						//参数内存界限
	proc->task.memmap.kernel_lin_base = KernelLinBase;					//内核基址
	proc->task.memmap.kernel_lin_limit = KernelLinBase + KernelSize;		//内核大小初始化为8M
	...
	//p_proc_current->task.info.text_hold = 1;			//是否拥有代码	//deleted by mingxuan 2019-1-16
	//p_proc_current->task.info.data_hold = 1;			//是否拥有数据	//deleted by mingxuan 2019-1-16
	proc->task.info.text_hold = 1;			//是否拥有代码
	proc->task.info.data_hold = 1;			//是否拥有数据
